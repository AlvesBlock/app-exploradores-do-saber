<template>
  ...
  >
    <div class="runner-mobile-stage" :style="{ backgroundImage: `url(${currentBackground})` }">
      ...
      <header class="runner-hud">
        ...
        <div class="hud-center">
          <div class="hud-progress-top">
            <span
              >Meta {{ gameState.stats.qualifiedCollects }}/{{
                gameState.stats.minCollectibles
              }}</span
            >
          ...
          <div class="hud-progress-bottom">
            ...
            <span
              >{{ Math.floor(gameState.stats.distance) }}/{{ gameState.stats.targetDistance }}</span
            >
          </div>
          <div class="shield-row">
            <span
              >Escudo {{ gameState.player.shieldCharge }}/{{
                gameState.player.shieldChargeNeeded
              }}</span
            >
      ...
      <div class="runner-road" @touchstart.passive="onTouchStart" @touchend.passive="onTouchEnd">
        ...
        >
          ...
          <span v-else-if="entity.collectibleClass === 'risky'" class="entity-tag risky"
            >CUIDADO</span
          >
          <span v-else-if="entity.collectibleClass === 'bad'" class="entity-tag bad"
            >FORA META</span
          >
        </div>
        ...
        <div
          class="runner-player"
          :class="{ shield: gameState.player.shieldActive }"
          :style="playerStyle"
        >
          {{ currentVehicleEmoji }}
        </div>
          ...
          <div class="pause-actions">
            ...
            <button
              v-if="
                ...
                gameState.roundProgress.finalResult?.canAdvanceToNextRound
              "
              class="primary-cta"
...
import {
  canUseEmergencyHeal,
  getVehicleById,
  getVehicleLaneSwitchMultiplier,
...
const currentVehicle = computed(
  () => getVehicleById(gameState.meta.selectedVehicleId) ?? RUNNER_VEHICLES[0],
)
const currentVehicleEmoji = computed(() => currentVehicle.value?.emoji ?? '🛹')
const currentBackground = computed(
  () => backgrounds[gameState.roundProgress.currentRoundConfig.background] ?? runnerCityBg6,
)
const availableVehicles = computed(() =>
  RUNNER_VEHICLES.filter((vehicle) => gameState.meta.unlockedVehicleIds.includes(vehicle.id)),
)
const progressPercent = computed(() =>
  Math.min(
    100,
    Math.round((gameState.stats.distance / Math.max(1, gameState.stats.targetDistance)) * 100),
  ),
)
const shieldChargePercent = computed(() =>
  Math.min(
    100,
    Math.round(
      (gameState.player.shieldCharge / Math.max(1, gameState.player.shieldChargeNeeded)) * 100,
    ),
  ),
)
...
const emergencyHealCostLabel = computed(
  () => `${gameState.roundProgress.currentRoundConfig.emergencyHealCost}c`,
)
const canSelectPreviousRound = computed(() => gameState.roundProgress.currentRound > 1)
const canSelectNextRound = computed(
  () => gameState.roundProgress.currentRound < gameState.meta.highestUnlockedRound,
)

const playerStyle = computed(() => {
  const leftX = ROAD_PROJECTION.topLeftX + (ROAD_PROJECTION.bottomLeftX - ROAD_PROJECTION.topLeftX)
  const rightX =
    ROAD_PROJECTION.topRightX + (ROAD_PROJECTION.bottomRightX - ROAD_PROJECTION.topRightX)
...
function getEntityStyle(lane: LaneIndex, depth: number) {
  ...
  const leftX =
    ROAD_PROJECTION.topLeftX + (ROAD_PROJECTION.bottomLeftX - ROAD_PROJECTION.topLeftX) * d
  const rightX =
    ROAD_PROJECTION.topRightX + (ROAD_PROJECTION.bottomRightX - ROAD_PROJECTION.topRightX) * d
  const roadWidth = rightX - leftX
  const laneWidth = roadWidth / ROAD_PROJECTION.laneCount
...
function getRoadMarkerStyle(depth: number) {
  ...
  const yPx = ROAD_PROJECTION.horizonY + (ROAD_PROJECTION.bottomY - ROAD_PROJECTION.horizonY) * d
  const halfRoadWidth =
    ROAD_PROJECTION.roadHalfWidthFar +
    (ROAD_PROJECTION.roadHalfWidthNear - ROAD_PROJECTION.roadHalfWidthFar) * d
...
function handleFeedback(event: RunnerFeedbackEvent) {
  ...
  flashTone.value =
    event.tone === 'negative' ? 'negative' : event.tone === 'warning' ? 'warning' : 'positive'
...
const { moveLeft, moveRight } = useRunnerInput(gameState, {
  ...
  getLaneSwitchCooldownMs: () =>
    Math.max(90, Math.round(140 / getVehicleLaneSwitchMultiplier(gameState.player.vehicleId))),
})
...
.runner-stage-overlay {
  ...
  background: linear-gradient(
    180deg,
    rgba(203, 233, 255, 0.22) 0%,
    rgba(255, 255, 255, 0.02) 38%,
    rgba(248, 243, 232, 0.18) 100%
  );
  z-index: 0;
}
.runner-stage-ground-glow {
  ...
  background: radial-gradient(circle, rgba(0, 0, 0, 0.16) 0%, rgba(0, 0, 0, 0) 74%);
  filter: blur(20px);
  z-index: 0;
}
.runner-hud {
...
.road-marker {
  position: absolute;
  border-radius: 999px;
  background: linear-gradient(180deg, rgba(255, 255, 255, 0.94) 0%, rgba(255, 255, 255, 0.7) 100%);
  box-shadow:
...
.entity-icon {
  font-size: 2rem;
  filter: drop-shadow(0 6px 8px rgba(0, 0, 0, 0.18)) drop-shadow(0 0 6px rgba(255, 255, 255, 0.06));
}
.entity-tag {
  padding: 4px 8px;
  font-size: 0.58rem;
...
.runner-entity.collectible.good .entity-icon {
  filter: drop-shadow(0 0 14px rgba(74, 222, 128, 0.48)) drop-shadow(0 6px 8px rgba(0, 0, 0, 0.18));
}
.runner-entity.collectible.risky .entity-icon {
  filter: drop-shadow(0 0 14px rgba(96, 165, 250, 0.48)) drop-shadow(0 6px 8px rgba(0, 0, 0, 0.18));
}
.runner-entity.collectible.special .entity-icon {
  filter: drop-shadow(0 0 16px rgba(251, 191, 36, 0.58)) drop-shadow(0 6px 8px rgba(0, 0, 0, 0.18));
}
.runner-entity.collectible.bad .entity-icon {
  filter: drop-shadow(0 0 12px rgba(244, 114, 182, 0.44)) drop-shadow(0 6px 8px rgba(0, 0, 0, 0.18));
}
.runner-entity.obstacle.minor .entity-icon {
  filter: drop-shadow(0 0 14px rgba(251, 191, 36, 0.38)) drop-shadow(0 6px 8px rgba(0, 0, 0, 0.18));
}
.runner-entity.obstacle.major .entity-icon {
  filter: drop-shadow(0 0 16px rgba(248, 113, 113, 0.42)) drop-shadow(0 6px 8px rgba(0, 0, 0, 0.18));
}
.runner-player {
  position: absolute;
...
.runner-player.shield {
  filter: drop-shadow(0 0 16px rgba(67, 214, 126, 0.9)) drop-shadow(0 8px 10px rgba(0, 0, 0, 0.22));
}
.runner-flash {
  position: absolute;
  inset: 0;
...
.runner-flash.positive {
  background: radial-gradient(
    circle,
    rgba(67, 214, 126, 0.22) 0%,
    rgba(67, 214, 126, 0.08) 45%,
    rgba(67, 214, 126, 0) 75%
  );
}
.runner-flash.warning {
  background: radial-gradient(
    circle,
    rgba(255, 189, 89, 0.24) 0%,
    rgba(255, 189, 89, 0.08) 45%,
    rgba(255, 189, 89, 0) 75%
  );
}
.runner-flash.negative {
  background: radial-gradient(
    circle,
    rgba(255, 97, 97, 0.28) 0%,
    rgba(255, 97, 97, 0.1) 45%,
    rgba(255, 97, 97, 0) 75%
  );
}
.runner-feedback-text {
  position: absolute;
  top: calc(max(10px, env(safe-area-inset-top)) + 148px);
  left: 50%;
  transform: translateX(-50%);
  z-index: 25;
  display: inline-flex;
...
.runner-feedback-text.positive,
.runner-feedback-text.neutral {
  background: #ebfff2;
  color: #177245;
}
.runner-feedback-text.warning {
  background: #fff8e2;
  color: #9a6700;
}
.runner-feedback-text.negative {
  background: #fff0f0;
  color: #9c2d2d;
}
...
.ghost-cta {
  ...
  color: #49566d;
  border: 2px solid rgba(73, 86, 109, 0.18);
}
.runner-controls {
  position: absolute;
  left: 0;
...
@media (max-width: 960px) {
  ...
  .runner-status-overlay {
    top: calc(max(10px, env(safe-area-inset-top)) + 164px);
  }
}
